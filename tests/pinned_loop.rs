...
fn gf128_pinned_loop_n12() {
    ...
    for i in 0..200 {
        ...
        let fused = i & 1 == 1;
        sumcheck_deg2_delayed_gf128_pinned(&mut f, &mut g, &challenges, fused);
        assert_eq!(f.len(), 1, "iter {i} (fused={fused}): bad final length");
    }
}
...
fn fp128_pinned_loop_n12() {
    ...
    for i in 0..200 {
        ...
        let mut g = g_orig.clone();
        let fused = i & 1 == 1;
        sumcheck_deg2_delayed_fp128_pinned(&mut f, &mut g, &challenges, fused);
        assert_eq!(f.len(), 1, "iter {i} (fused={fused}): bad final length");
    }
}
