...
//!
//! This module is the prototype for the future `pinned-pool` and
//! `sumcheck-parallel` crates (see
//! `docs/plans/sumcheck-parallel-productionization.md`). It is split
//! into:
//!
//! - [`pool`]: process-global persistent worker pool with QoS pinning
//!   and a Release/Acquire epoch broadcast (Approach 4).
...
//!   field-agnostic driver that calls `pool::broadcast_scoped` once
//!   and contains the D1 (tournament reduce) and D2 (per-round
...
//!   repo, plus the back-compat function wrappers
